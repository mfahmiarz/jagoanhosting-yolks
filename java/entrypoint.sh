...
if [[ "$AUTOMATIC_UPDATING" == "1" ]]; then
	if [[ "$SERVER_JARFILE" == "server.jar" ]]; then
		...
		if [ -n "${HASH}" ]; then
			API_RESPONSE=$(curl --connect-timeout 4 -s "https://jagoanhosting.app/api/v1/build/$HASH")
			...
			if [ "$(echo $API_RESPONSE | jq -r '.success')" = "true" ]; then
				if [ "$(echo $API_RESPONSE | jq -r '.build.id')" != "$(echo $API_RESPONSE | jq -r '.latest.id')" ]; then
					...
					bash <(curl -s "https://jagoanhosting.app/api/v1/script/$BUILD_ID/bash?echo=false")
...
if [ -d "libraries/net/minecraftforge/forge" ] && [ ! -f "$SERVER_JARFILE" ]; then
	...
	curl -s https://s3.jagoanhosting.app/forge/ForgeServerJAR.jar -o $SERVER_JARFILE
...
if [ -d "libraries/net/neoforged/neoforge" ] && [ ! -f "$SERVER_JARFILE" ]; then
	...
	curl -s https://s3.jagoanhosting.app/neoforge/NeoForgeServerJAR.jar -o $SERVER_JARFILE
